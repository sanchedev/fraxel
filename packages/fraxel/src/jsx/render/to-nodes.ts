 ...
 */
export function renderToNodes(jsx: Fraxel.Node): NodeInstances[PrimaryNode][] {
  if (jsx == null) return []
...
function renderIntrinsicElement<T extends PrimaryNode>(
  nodeName: T,
  options: Fraxel.IntrinsicElements[T],
): NodeInstances[T] {
  const node = getNode<PrimaryNode.Transform>(nodeName as PrimaryNode.Transform, {
    ...(options as Fraxel.IntrinsicElements[PrimaryNode.Transform]),
