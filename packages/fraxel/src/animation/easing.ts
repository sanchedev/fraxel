/**
 * The **`EasingFn`** type represents an easing function that maps progress `t`
 * (0 to 1) to an eased value. Used by `tween()` to control animation curves.
 */
...
/** **`easeInOutQuad`** — Accelerating then decelerating. */
...
/** **`easeBackOut`** — Overshoot then settle, dramatic entrance. */
export const easeBackOut: EasingFn = (t) => {
  const c = 1.70158
  return 1 + --t * t * ((c + 1) * t + c)
}

/** **`easeBackInOut`** — Overshoot in both directions, bouncy. */
...
/** **`easeOutElastic`** — Elastic effect, spring-like overshoot. */
